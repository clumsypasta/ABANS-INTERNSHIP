...
with st.sidebar:
    st.header("About")
    st.markdown("""
    This application uses Google's ADK agents communicating over the A2A protocol to:

    1. Parse trade data files
    2. Net buy/sell trades per contract
    3. Compute FIFO and LIFO weighted average prices
    4. Fetch current market prices (optional)
    5. Generate visualizations
    6. Provide AI-powered insights and answers

    The system consists of six agents:
    - Frontend Agent (Streamlit UI)
    - Data Agent (Parsing)
    - Compute Agent (Netting & WAP)
    - Pricing Agent (Market Prices)
    - Visualization Agent (Charts & Tables)
    - AI Assistant Agent (Insights & Q&A)
    """)
